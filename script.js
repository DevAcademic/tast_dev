(function() {
    ...
    window.addManualCode = function() {
        ...
        teacher.codes.push({ code: code, used: false, deviceId: null });
        saveData();
        pendingChanges++;
        updatePendingChanges();
    ...
    function getCodesStatus(teacher) {
        ...
        return {
            total: teacher.codes.length,
            used: teacher.codes.filter(c => c.used).length,
            available: teacher.codes.filter(c => !c.used).length
        };
    }
    ...
    document.getElementById('deleteSemesterTeacher').addEventListener('change', function() {
        ...
        const deptIndex = selectedOption ? parseInt(selectedOption.dataset.dept) : -1;
        const teacherIndex = parseInt(this.value);
        const semesterSelect = document.getElementById('deleteSemesterSelect');
        let options = '<option value="">اختر الفصل...</option>';
        if (deptIndex !== -1 && data.departments[deptIndex]) {
            const teacher = data.departments[deptIndex].teachers[teacherIndex];
    ...
    window.activateCodeFromTeacher = function() {
        ...
        if (result.valid) {
            ...
            setTimeout(() => {
                ...
                if (deptIndex !== -1 && activeTeacherIndex !== null) {
                    openTeacher(deptIndex, activeTeacherIndex);
                }
            }, 1500);
        }
    };
    ...
    function updateCodesManagement() {
        ...
        let html = `
            ...
            <table class="codes-table">
                <tr><th>#</th><th>الكود</th><th>الحالة</th><th>حذف</th></tr>
        `;
        if (teacher.codes && teacher.codes.length > 0) {
            teacher.codes.forEach((c, index) => {
                ...
                html += `
                    <tr>
                        ...
                        <td>${statusText}</td>
                        <td>${!isUsed ? `<button onclick="deleteThisCode(${deptIndex}, ${teacherIndex}, '${c.code}')" style="background:#ef4444;color:white;border:none;border-radius:4px;padding:0.2rem 0.6rem;cursor:pointer;">🗑️</button>` : '—'}</td>
                    </tr>
                `;
            });
        } else {
    ...
    addTeacherForm.addEventListener('submit', function(e) {
        ...
        const newTeacher = {
            name, emoji, subject: subject || '', description: description || '', image: image || '',
            codes: [], semesters: []
        };
    ...
    addSemesterForm.addEventListener('submit', function(e) {
        ...
        const newSemester = { number, description: description || `الفصل ${number}`, lectures: [] };
    ...
    publishBtn.addEventListener('click', function() {
        ...
        message.textContent = '✅ تم نشر التغييرات بنجاح! تم تحميل ملف data.json';
